  ...
  describe('IsLiteral<T> extends true', () => {
    ...
    describe('S extends string', () => {
      it('should equal T.split(S)', () => {
        // Arrange
        type T1 = EmptyString
        type T2 = Lowercase<'0.x.y'>

        // Expect
        expectTypeOf<TestSubject<T1, Dot>>().toEqualTypeOf<[T1]>()
        expectTypeOf<TestSubject<T1, EmptyString>>().toEqualTypeOf<EmptyArray>()
        expectTypeOf<TestSubject<T2, Sep>>().toEqualTypeOf<[T2]>()
        expectTypeOf<TestSubject<T2, Dot>>().toEqualTypeOf<['0', 'x', 'y']>()
      })
